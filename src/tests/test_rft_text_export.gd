...
func _run():
	...
	assert(parsed2.get("reference_json", {}).get("do_function_call", false))
	assert(parsed2.get("reference_json", {}).get("ideal_function_call_data", {}).get("name", "") == "add")
	var parsed3 = JSON.parse_string(lines[2])
	assert(parsed3.get("reference_json", {}).get("foo", "") == "bar")
	assert(parsed3.get("reference_json", {}).get("do_function_call", true) == false)
